...
| `FSA_DELAY_MS` | `1500` | Задержка между запросами списка деклараций (мс) |
| `FSA_DETAIL_DELAY_MS` | `300` | Задержка перед запросом карточки конкретной декларации (мс). ФСА стоит за антифрод-WAF (Solar JSOC) — при резком увеличении объёма запросов (например, после смены `FSA_DATE_CHUNK` или снятия фильтров) IP сервера может быть забанен/затроттлен на их стороне (TCP-коннект на 443 зависает, при этом ICMP продолжает отвечать). Если это произошло — подождать (бан временный) и увеличить эту задержку перед повторным запуском |
