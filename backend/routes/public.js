...
<style>
  body{font-family:-apple-system,Segoe UI,Arial,sans-serif;max-width:640px;margin:40px auto;padding:0 16px;color:#1a1e27;line-height:1.5}
  ...
  .card b{display:block;font-size:11px;text-transform:uppercase;letter-spacing:.04em;color:#6b7280;margin-bottom:4px}
  .cta{background:#185FA5;color:#fff;text-decoration:none;padding:10px 18px;border-radius:8px;display:inline-block;font-weight:600;margin-top:8px}
  .prod{display:inline-block;background:#eef4ff;color:#185FA5;border-radius:6px;padding:3px 9px;font-size:12px;margin:2px 4px 2px 0}
  footer{margin-top:32px;font-size:12px;color:#9ca3af}
</style>
</head>
<body>
  <div class="brand">БАЗА <span>АПК</span></div>
  ${body}
  <footer>© 2026 База АПК · Агрегатор открытых данных pub.fsa.gov.ru, не аффилирован с Росаккредитацией</footer>
</body>
</html>`;

router.get('/company/:inn', (req, res) => {
  const inn = String(req.params.inn || '').replace(/\D/g, '');
  if (!inn) return res.status(404).send(PAGE_SHELL('Компания не найдена — База АПК', '', '<h1>Компания не найдена</h1>'));
  ...
  if (!rows.length) {
    res.status(404);
    return res.send(PAGE_SHELL('Компания не найдена — База АПК', 'В реестре действующих деклараций такая компания не найдена.',
  ...
  const title = `${name} — ${place} — База АПК`;
